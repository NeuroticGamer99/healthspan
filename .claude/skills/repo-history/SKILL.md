...
**Every column a category row is filtered on is now a column you can see**, so read a `+0` cell as that metric alone. The table carries `Δ Files`, `Δ Lines`, `Δ Code`, `Δ Comment`, `Δ Blank` and `Δ Bytes`, which is exactly the set "did this category change?" is decided on — so by default a *category* row always carries at least one non-zero cell, and under `--all` a category row of `+0` throughout means that category genuinely did not move.

**The `**Total**` row is not filtered and is not covered by that.** It is appended unconditionally, after every category row, so it reads `+0` across the board in at least two situations that have nothing to do with `--all`, both measured: when nothing changed at all (`diff HEAD HEAD` prints a table whose only row is an all-`+0` Total, under `_No category changed._`), and when category deltas **cancel** — `src/` losing a line while `tests/` gains one renders as `-1` and `+1` above a Total of `+0`. A zero Total is a statement about the net, never about the rows.

This guidance previously read "a `+0` row means files changed, net size did not". That was describing a *defect* — a byte-only change survived the filter into a table with no bytes column and rendered as `+0` across the board — rather than behaviour to rely on. Its first replacement then claimed an all-`+0` row appears "only under `--all`", which the Total row falsifies; the lesson worth keeping is that a claim about "a row" in this table has to say *which* rows, because one of them is exempt from the rule the others follow.
...
**When a revision's blob cannot be read, the exit-2 message names the object, the commit and the path.** The run refuses rather than warning because there the silence would be unrecoverable: the point would report a smaller repository than existed and nothing downstream could tell it from real shrinkage. The `scripts/repo_stats.py` module docstring holds the full reasoning — point there rather than re-deciding it.
