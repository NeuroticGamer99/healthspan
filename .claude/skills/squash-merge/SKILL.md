...
- **No bot finding is unanswered** — run this, do not eyeball it. The PR's `createdAt` is the
  ...
  **Exit 2 clears this gate**: every finding any bot posted has a threaded reply from someone
  ...
  Those are findings the sweep could not read, not findings that do not exist. For the two
  prose-finding shapes the banner prints the exit: an `Acknowledges <bot> (summary|review) <id>`
  PR-level comment answering them (`.claude/bot-review-triage.md` §2, ADR-0067) — an artifact so
  acknowledged no longer blocks, so re-running the sweep after posting it is how the gate clears.
  **Without** the
