...
WI-2 shipped kit rendering as terminal display plus explicit `--output` file (ADR-0047 §4). The **orphan startup sweep shipped in Phase 2 WI-1** (`recovery_kit.sweep_orphans`, called from Core Service startup; [ADR-0049](adr/0049-core-service-skeleton-implementation-decisions.md)) — it disposes any orphaned `*recovery-kit*` plaintext in the private data directory, best-effort (overwrite-then-unlink). Still deferred: ADR-0033's OS print integration (`lp`/`lpr` streaming, Windows temp-file shell print with verified disposal) — the sweep's only *producer* of orphans. The print pathways land with the user-documentation milestone; until then the sweep hook is a no-op backstop. Trigger: user-documentation milestone (or the first Windows shell-print pathway).
...
Phase 2 WI-1 shipped the ADR-0042 single-instance advisory lock (`healthspan/locking.py`) and applied it as `cli_support.exclusive_database_access` to the three sanctioned direct-database commands — `db migrate`, `db backup`, `db restore` — which now acquire `<database-path>.lock` fail-fast and refuse while the Core Service (or another instance) holds it ([ADR-0049](adr/0049-core-service-skeleton-implementation-decisions.md) §6). `db restore` holds it for its whole duration, per ADR-0042. **Still open:** (1) the `keys` rotation commands (`change-passphrase`/`rotate-secret-key`/`convert-mode`) should acquire the same guard per [ADR-0033](adr/0033-plaintext-artifact-disposal.md) — deferred to **WI-2** (they live in the crypto CLI WI-2 revisits); (2) the in-service scheduled `backup.database` job (ADR-0038's scheduled producer, of which the WI-4 CLI is the offline counterpart) — deferred until the job/scheduler surface exists.
