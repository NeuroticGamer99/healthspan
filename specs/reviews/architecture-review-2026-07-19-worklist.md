...
- [x] Split Phase 4 into 4a (launcher + event bus + jobs; milestone: imports run as jobs emitting `data.*`/`job.*` over SSE) and 4b (MCP + exports + ADR-0014 disposition; milestone: AI client queries real data with full fidelity), keeping both halves inside the immutable "Phase 4" binding. *Resolved: [development-plan.md](../development-plan.md) (R4) — one `## Phase 4` heading with `### Phase 4a` / `### Phase 4b` subsections, so the "subdivisions, not new phases" claim is visible in the structure; the split note cites the three Accepted ADRs whose immutability carries the binding argument (0052/0053/0055) plus Proposed ADR-0065, which references the number without carrying that argument. The launcher's own rationale had to be rewritten to survive the split: "the second process is the first thing worth orchestrating" is a 4b fact, so 4a justifies the launcher by ADR-0039's migration ownership and passphrase handoff instead — process-count-independent — leaving 4b to add a second child to an existing stack. Phase 2's forward pointer updated to match. **ADR-0014's disposition landed in 4a, not 4b as this bullet says** — the review's own rationale (4.A prose) and this item's note below both place it with the SSE work, and it is a WebSocket-vs-SSE question that nothing in MCP or exports informs; the checkbox text is left as written, per the no-rewriting-the-record convention.*
