...
- [x] Decide the defaults (warn vs. refuse per file class; Windows-check depth via `fsperm.py`'s explicit-principal enumeration), implement the checks at `build_runtime`/`exclusive_database_access` time, and add the testing-strategy targets. *Resolved: [ADR-0066](../adr/0066-startup-permission-verification.md) (R5) — the warn-vs-refuse line lands as **repair-vs-refuse**: the platform's own files (database, sidecar, backup directory, published backups, config) are restored to owner-only in place and the drift reported, while `passphrase_file` refuses and is never touched. Windows uses the explicit-principal enumeration as the review suggested, with a benign set (SYSTEM, Administrators, and the owner placeholders `OWNER RIGHTS`/`CREATOR OWNER`) resolved from well-known SIDs so the name match survives a localized install. No override flag or config key. Scope took the three named files plus the backup archive; the per-client token fallback the owner also asked for **does not exist in code yet** (`keychain.py`/`cli_client.py` are keyring-only), so it is recorded as a forward obligation in §5 rather than invented. Two things the implementation found: real Windows profile ACLs carry owner placeholders the review's two-identity sketch would have reported as exposures, and nine existing test fixtures were creating world-readable passphrase files — the refusal caught them all on the first run.*
...
- [x] Split Phase 4 into 4a (launcher + event bus + jobs; milestone: imports run as jobs emitting `data.*`/`job.*` over SSE) and 4b (MCP + exports + ADR-0014 disposition; milestone: AI client queries real data with full fidelity), keeping both halves inside the immutable "Phase 4" binding. *Resolved: [development-plan.md](../development-plan.md) (R4) — one `## Phase 4` heading with `### Phase 4a` / `### Phase 4b` subsections, so the "subdivisions, not new phases" claim is visible in the structure; the split note cites the three Accepted ADRs whose immutability carries the binding argument (0052/0053/0055) plus Proposed ADR-0065, which references the number without carrying that argument. **Two contents deviate from this bullet's text, which is left as written per the no-rewriting-the-record convention.** (1) **ADR-0014's disposition landed in 4a, not 4b** — the review's own rationale (4.A prose) and this item's note below both place it with the SSE work, and it is a WebSocket-vs-SSE question that nothing in MCP or exports informs. (2) **The launcher landed in 4b, not 4a** — [ADR-0049](../adr/0049-core-service-skeleton-implementation-decisions.md) §5 (Accepted) defers it to "Phase 4, when the MCP Server makes a second process to orchestrate", so it belongs beside the MCP Server; in 4a the stack is still a single process and the launcher would have nothing to orchestrate, which is exactly why Phase 2 deferred it. The phase-level deferral was never at risk (4a and 4b are both inside Phase 4) but the trigger clause is specific, and 4b honors it literally at no ADR cost. Phase 2's forward pointer updated to match. Caught by `spec-reviewer`; two /code-review passes, Greptile, CodeRabbit and Copilot had all missed the ADR-0049 §5 tie.*
- [x] Add the range-model expressiveness row to the decision-gates table (trigger: first hormone-panel range / first visibly-wrong flag; owner: database owner). *Resolved: row added, phrased "Trigger, not a phase" in the Needed-by column so it reads correctly beside the phase-numbered gates, and pointing at both open-questions.md (Schema) and lab-data-dimensions.md.*
- [x] Reword the Phase 3.5 catalog-seed bullet to tie the seed to the correction work (seed the concepts the source survey found duplicated or aliased, exercising the merge/alias tooling). *Resolved: the bullet's selector is now the survey's correction findings — duplicated/aliased-across-sources concepts, seeded with the alias rows that stop the duplicates recurring — with the "no seed row for a concept the survey never saw collide" bound stated explicitly. Unit-validation and conservative-range-values text kept verbatim.*
