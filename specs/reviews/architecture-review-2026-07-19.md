...
- [x] Implement the on-open permission checks security.md already requires, and add the testing-strategy targets.
...
2. **Natural-key collision.** `ux_lab_results_natural_key` is a partial unique index on `(lab_draw_id, biomarker_id) WHERE superseded_by IS NULL` (`src/healthspan/migrations/0003_import_conflict_keys.sql` line 28). If one draw holds current results for both the orphan and the survivor — precisely the same-concept-duplicate scenario that motivates merge — re-pointing violates the index. The ADR must define the outcome (refuse? supersede the orphan's result as a duplicate? merge values?), because the DB will otherwise define it as an `IntegrityError` at runtime.
...
- [x] Split Phase 4 into 4a (launcher + event bus + jobs; milestone: imports run as jobs emitting `data.*`/`job.*` over SSE) and 4b (MCP + exports + ADR-0014 disposition; milestone: AI client queries real data with full fidelity). *Resolved: [development-plan.md](../development-plan.md) (R4) — `### Phase 4a` / `### Phase 4b` under the retained `## Phase 4` heading, with the split note citing the three Accepted ADRs whose immutability binds the number (0052/0053/0055) alongside Proposed ADR-0065, which references it without binding it. Two contents deviate from this bullet's text. The ADR-0014 disposition went to **4a**, following this item's own rationale paragraph below ("with the SSE work it depends on") rather than this bullet's parenthetical — it is answerable the moment the SSE stream exists and nothing in MCP or exports informs it. The **launcher went to 4b**, not 4a: [ADR-0049](../adr/0049-core-service-skeleton-implementation-decisions.md) §5 (Accepted) defers it to "Phase 4, when the MCP Server makes a second process to orchestrate", and in 4a the stack is still a single process — the same reasoning Phase 2 used to defer it. The deferral to "Phase 4" was satisfied either way (both halves are inside it); 4b additionally honors the trigger clause, at no ADR cost.*
...
- [x] Add a row: "Range-model expressiveness (cohort dimensions, exclusive bounds, `physical_min`, non-numeric comparison) — trigger: first hormone-panel range / first visibly-wrong flag — owner: database owner." *Resolved: row added to the decision-gates table (R4), with "Trigger, not a phase" in the Needed-by column and pointers to open-questions.md (Schema) and lab-data-dimensions.md.*
...
- [x] Tie the "fuller built-in catalog" bullet explicitly to the correction work: seed the concepts the source survey found duplicated or aliased, so the seed exercises the merge/alias tooling — rather than an open-ended "more useful" seed. *Resolved: bullet retitled "A fuller built-in catalog, bounded by the correction work" (R4); the selector is now the survey's correction findings, and the bound is stated as a negative too — a concept the survey never saw collide earns no seed row in this phase.*
