...
- Startup sequence and passphrase handoff ([ADR-0039](adr/0039-startup-sequence-and-passphrase-handoff.md)); single-instance locking ([ADR-0042](adr/0042-process-supervision-and-single-instance-locking.md)). This phase ships **direct-start only** (`healthspan service start`, foreground) — with a single Core Service process there is nothing for a launcher to orchestrate. The launcher (ADR-0039's migration ownership + passphrase-over-stdin handoff) arrives in Phase 4a — early, because that ownership is worth having before the MCP Server makes it the second orchestrated process in 4b; full supervision and full-auto-unlock arrive with Phase 6 ([ADR-0049](adr/0049-core-service-skeleton-implementation-decisions.md)).
...
*The halves are **subdivisions, not new phases**. The number 4 stays bound to "the event-bus/MCP surface", because other ADRs reference it by that number — [ADR-0052](adr/0052-bulk-import-identity-and-conflict-resolution.md), [ADR-0053](adr/0053-read-endpoint-surface-and-pagination.md) and [ADR-0055](adr/0055-biomarker-category-taxonomy.md) are Accepted and therefore immutable, so a renumber would falsify them with no way to edit them to follow; [ADR-0065](adr/0065-catalog-merge-and-removal.md) (Proposed) cites it too. Any such reference is satisfied by the pair. 4a lands first — 4b's tools are clients of the event and job surfaces 4a establishes. Each half ends at its own crisp, testable milestone, per this plan's own rule.*

### Phase 4a — Launcher, event bus, jobs

- The **launcher** ([ADR-0039](adr/0039-startup-sequence-and-passphrase-handoff.md), [ADR-0008](adr/0008-process-lifecycle.md)): it lands in 4a for the half of its job that does not depend on how many processes it starts — **migration ownership and the passphrase handoff**. ADR-0039 makes the launcher the definitive migration owner (Core Service verifies `schema_version` and exits on mismatch, never migrates) and routes the passphrase to Core over stdin; today the CLI's `healthspan db migrate` carries migrations, and ADR-0035's exclusive-access argument is only literally true once the launcher owns them. So 4a is where the launcher takes over migrations, derives-and-discards, and starts a one-child stack in health-gated dependency order ([observability.md](observability.md)); 4b adds the MCP Server as a second child to a stack that already exists, rather than introducing orchestration and the second process at once. Phase 2 shipped direct-start (`healthspan service start`) instead; full supervision (restart-with-backoff, circuit breaker) and full-auto-unlock are deferred to Phase 6 ([ADR-0042](adr/0042-process-supervision-and-single-instance-locking.md), [ADR-0049](adr/0049-core-service-skeleton-implementation-decisions.md)).
...
**Milestone:** an import runs as a job and a subscribed client observes it live — `job.*` and `data.*` events delivered over the SSE stream, on a stack the launcher migrated and started.

### Phase 4b — MCP server and exports
