...
- Startup sequence and passphrase handoff ([ADR-0039](adr/0039-startup-sequence-and-passphrase-handoff.md)); single-instance locking ([ADR-0042](adr/0042-process-supervision-and-single-instance-locking.md)). This phase ships **direct-start only** (`healthspan service start`, foreground) — with a single Core Service process there is nothing for a launcher to orchestrate. The launcher (ADR-0039's migration ownership + passphrase-over-stdin handoff) arrives in Phase 4b with the MCP Server, the second process ([ADR-0049](adr/0049-core-service-skeleton-implementation-decisions.md) §5); full supervision and full-auto-unlock arrive with Phase 6 ([ADR-0049](adr/0049-core-service-skeleton-implementation-decisions.md)).
...
Phase 3 delivered catalog **growth** (add a biomarker / lab / alias) but no catalog **correction**, and `enter` captures a result's value but not the lab's own printed reference range. The first real report exercised both gaps within one session: a same-concept duplicate biomarker with no in-app way to remove or merge it, and lab-provided ranges dropped on entry. These are not Phase-3 bugs — they are a horizontal **data-lifecycle** concern (catalog hygiene, entry fidelity) that recurs at every later data ingress. Left unhardened they also block Phase 4b, whose milestone is MCP over real data *with full fidelity* — which needs a clean, correctable dataset, and manual entry is the practical ingress for the owner's own reports until Phase 7's source adapters. (The Phase-2 bulk-import API is a live authenticated write path too, but it already accepts the `reference_low` / `reference_high` / `reference_text` columns — `imports.py` — so the reference-range fidelity gap is specific to the `enter` client, not the write path.)

Scope — deliberately bounded to the orphan, **not** a second Phase 3:

- **Catalog correction** — semantics decided in [ADR-0065](adr/0065-catalog-merge-and-removal.md) (Proposed, 2026-07-24): `biomarkers` / `labs` **remove** (blocked while observations reference the entry; the entry's own aliases and ranges cascade, audited) and **merge** (re-point every referencing row — `lab_results`, `biomarker_aliases`, `framework_ranges` for a biomarker, `lab_draws` for a lab — onto a survivor, delete the orphan, alias the merged-away biomarker name), plus first-class alias management, closing the [open-questions.md](open-questions.md) "CLI catalog editing" deferral and the alias-vs-duplicate trap that produces same-concept duplicates. The merge work item also ships the collision-resolution surfaces ADR-0065 §3 requires (result deletion; audited framework-range deletion; duplicate-draw cleanup) and records its command surface as an ADR-0060 edit.
...
- **A fuller built-in catalog, bounded by the correction work** — the seed's selector is the survey's *correction* findings, not open-ended usefulness: seed the concepts the source survey found **duplicated or aliased across sources**, so the starter catalog carries the alias rows that stop those duplicates being re-created by hand, and the seeded set is the material the merge/alias tooling above is exercised against. A concept the survey never saw collide earns no seed row in this phase — "it would be nice to have" is the growth argument Phase 3.5 is explicitly not making, and this bullet is the one item whose discriminator would otherwise be growth rather than correction. Names and categories are risk-free reference data. Units are *not* risk-free — a seeded `canonical_unit` must be valid UCUM and convertible, the same check `test_migration_0005` already enforces for seeded range units — so they earn that validation, not a free pass. Range *values* stay conservative — only the validated cells — because a wrong range mis-flags where a missing one merely stays silent.
...
**Scoping instrument — the source survey.** Phase 3.5's precise work items are finalized by a deliberate **discovery activity**: reviewing the owner's real reports across all sources to enumerate the full set of data dimensions up front, rather than migrating the schema reactively one report at a time. Its output is [lab-data-dimensions.md](lab-data-dimensions.md) — a generic domain model of value shapes, range dimensions, catalog-structure needs, and source-format quirks, each marked *validated by real data* vs *known-needed but unexercised* (the axes any single person's corpus cannot exercise on its own — the cohort dimensions it does not occupy, the unit defaults of labs it never used, specialty result types it never met — enumerated from domain knowledge, never from the corpus). The survey enumerates *reality*; its **decisions** route to ADRs / [data-model.md](data-model.md) / [open-questions.md](open-questions.md) as usual, so it never becomes a shadow spec.

**Containment.** The survey analyzes real reports **in chat or in the personal directory outside the repository** (its raw corpus is staged there, under `ingestion/`) and writes only the *structural distillation* — value shapes, dimensions, biomarker names/units, source-format quirks — to the public survey. Personal values, result patterns, and identifiers never enter the repository. This is the [CLAUDE.md](../CLAUDE.md) containment rule applied to a discovery activity; the `/land` containment gate and `spec-reviewer` are the backstops.

**Milestone:** the manual-entry catalog is correctable (no unfixable duplicates) and entry is faithful (lab ranges captured) — a clean, trustworthy dataset ready to be Phase 4b's MCP substrate.

## Phase 4 — AI surface: events, jobs, MCP

*Split into halves 4a and 4b on 2026-07-25 ([architecture review 2026-07-19](reviews/architecture-review-2026-07-19.md), 4.A). Phases 1–3 were each a single vertical; Phase 4 as originally written bundled five first-of-their-kind subsystems — the launcher (first multi-process orchestration), the event bus and SSE (first async surface), jobs (first long-running-work surface), the MCP server and its tool output contract, and the export endpoints — behind one milestone, each with its own security surface.*

*The halves are **subdivisions, not new phases**. The number 4 stays bound to "the event-bus/MCP surface", because other ADRs reference it by that number — [ADR-0052](adr/0052-bulk-import-identity-and-conflict-resolution.md), [ADR-0053](adr/0053-read-endpoint-surface-and-pagination.md) and [ADR-0055](adr/0055-biomarker-category-taxonomy.md) are Accepted and therefore immutable, so a renumber would falsify them with no way to edit them to follow; [ADR-0065](adr/0065-catalog-merge-and-removal.md) (Proposed) cites it too. Any such reference is satisfied by the pair. 4a lands first — 4b's launcher and tools are clients of the event and job surfaces 4a establishes. Each half ends at its own crisp, testable milestone, per this plan's own rule.*

### Phase 4a — Event bus and jobs
...
- **Disposition of [ADR-0014](adr/0014-websocket.md)** (WebSocket — Proposed, never on a flip list): the SSE implementation work makes it naturally resolvable; expected outcome is supersession/subsumption by [ADR-0011](adr/0011-event-bus.md). Mechanics decided 2026-07-17: since ADR-0011 is Accepted and immutable (it cannot absorb the rationale itself), the disposition is recorded as a **small new ADR that supersedes ADR-0014** — the new ADR carries the one-line rationale, 0014's status flips to `Superseded by ADR-XXXX`, and 0011 gains a navigation link. It sits in 4a, not 4b: it is a WebSocket-versus-SSE question, answerable the moment the SSE stream exists, and nothing in the MCP or export work informs it.

**Milestone:** an import runs as a job and a subscribed client observes it live — `job.*` and `data.*` events delivered over the SSE stream.

### Phase 4b — Launcher, MCP server, exports

- The **launcher** ([ADR-0039](adr/0039-startup-sequence-and-passphrase-handoff.md), [ADR-0008](adr/0008-process-lifecycle.md)): the second process (the MCP Server, below) is the first thing worth orchestrating, so the launcher lands in the same half — it owns migration execution, hands the passphrase to Core Service over stdin, and starts the stack in health-gated dependency order ([observability.md](observability.md)). This is the placement [ADR-0049](adr/0049-core-service-skeleton-implementation-decisions.md) §5 (Accepted) specifies — the launcher is deferred to "Phase 4, **when the MCP Server makes a second process to orchestrate**" — so it belongs with the MCP Server rather than in 4a, where the stack would still be a single process and the launcher would have nothing to orchestrate. Phase 2 shipped direct-start (`healthspan service start`) instead and keeps migration ownership with the operator (ADR-0049 §5); full supervision (restart-with-backoff, circuit breaker) and full-auto-unlock are deferred to Phase 6 ([ADR-0042](adr/0042-process-supervision-and-single-instance-locking.md), ADR-0049).
...
| Range-model expressiveness — cohort dimensions (sex/age/fasting), exclusive bounds, `physical_min`, non-numeric comparison ([open-questions.md](open-questions.md), Schema; [lab-data-dimensions.md](lab-data-dimensions.md)) | Trigger, not a phase: the first hormone-panel range entered, or the first visibly-wrong range flag | Database owner |
| [ADR-0014](adr/0014-websocket.md) disposition | Phase 4a (during SSE work) | Database owner |
