...
`scripts/check_doc_citations.py` asserts, per owning document, that the document exists and that every file registered as citing it still cites it. **The registry's size is deliberately not stated here.** This sentence used to carry it — "six citations across two documents today" — and by the time anyone noticed, the registry had reached **thirteen citations across three owning documents** with the sentence unchanged. That figure is the evidence and it is safely historical: it is what the count had silently drifted to *before* the change that deleted it, and it cannot move again. The same change adds a fourth owning document — deliberately, so do not read that growth as more drift — and the current size is not restated here, because a sentence arguing that live counts rot has no business carrying one. A count of a set this document does not own is a claim that decays on somebody else's schedule, so read it from the two places that are kept true by construction: the gate prints it on every green run, and `_EXPECTED_PAIRS` in `tests/test_check_doc_citations.py` pins the membership (with `_EXPECTED_NEEDLES` pinning the needles). It runs in the `docs-consistency` job beside `scripts/check_reviewer_agents.py`, whose citation assertion is the same shape for `.claude/reviewer-isolation.md`.

**"Still cites it" is the document's path plus, for some rows, a registered *needle* — an extra literal the citation must carry.** The path alone proved insufficient the first time it was relied on: `/apply-review` already contained `.claude/bot-review-triage.md` before this change, in an unrelated §4 reference, so the row added to gate §1's peer-search rule was satisfied by a mention that had nothing to do with it. Measured, not reasoned: deleting the entire cited step left the gate exiting 0 and reporting six healthy citations — the registry's size at that measurement, recorded as history and not to be updated with it. The needle for that row is the bold lead-in of the bullet being cited — not §1's heading, which names the wider procedure and would be no more distinguishing than the path.
...
- The gate's registry is hand-maintained, so a new caller that forgets both the citation and its registry row passes — the discovery blind spot the gate's own docstring names. Adding a caller was a two-file edit when this ADR was written; BRIEF-3 pinned the registry's *membership* in `tests/test_check_doc_citations.py` (`_EXPECTED_PAIRS`, resolving the "Pin the citation registry's membership" open question), so it is now a **three-file** edit — the registry, the caller's citation, and the pin. That cost is the point rather than a regression: an unpinned addition and a silent row deletion both redden a named test, where before a deleted row deleted its own test case.
...
- Con: `CLAUDE.md` is loaded into every session's context; a presentation contract and its rationales are a poor use of that budget when a citation costs one line — and the contract grows, so the budget argument only sharpens. (It said "four rules" until rule 5 landed, which is why it now names none: the count is not what the argument rests on, and carrying it here meant restating §1's rule list in a section nobody updates.)
