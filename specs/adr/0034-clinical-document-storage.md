...
- **MCP exposure**: the queryable surface for AI clients is the extracted `body` text; binary originals are not exposed through MCP tools by default. `body` text is not bounded in size (a scanned document's full OCR extraction can run far longer than a typical row), so a document-body tool is subject to the same pagination/row-cap discipline as any other tool output ([api-reference.md](../api-reference.md) rule 3) — returned as a bounded, cursor-paginated chunk rather than one unbounded string.

### Closing the ingestion loop (ADR-0033 tie-in)
After a successful import — blob stored, hash verified against the source — the CLI importer offers disposal of the *source* file under the [ADR-0033](0033-plaintext-artifact-disposal.md) policy: user-confirmed best-effort disposal, honestly caveated, FDE recommendation printed. The watch-folder importer, with no one to ask, applies its config-declared post-import action instead — move to `processed/` (the default), keep, or ADR-0033 disposal with the caveat logged ([ADR-0025](0025-plugin-host-process-matrix.md), review 2026-07-07 item 2.4). Declining is fine (the source is user-custody plaintext, like an export); the point is that the platform makes "the only durable copy is encrypted" a one-keystroke outcome instead of leaving a plaintext copy behind by default silence.
