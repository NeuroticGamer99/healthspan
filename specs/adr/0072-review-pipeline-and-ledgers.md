...
- **Revision stamp.** The brief carries one, and the report **echoes it back**. This is not optional while the brief is handed over by path.
- **Expiry.** The brief records when it was written and against what tree state; `/review-prep` checks it. The two anchors are **additive, not alternative**: the `git status --porcelain` digest is checked **always**, and the `HEAD` SHA **additionally** when `HEAD` differs from the base. Stated as an either/or it has a hole — a branch that has commits *and* uncommitted edits would take the SHA path alone, and edits made after the brief was written move no SHA, so an expired brief passes as current. That is the committed-anchor-standing-in-for-a-dirty-tree class `/review-prep` already calls worse than a missing anchor. The digest is the load-bearing half (a zero-commit branch has `HEAD == origin/main`, so the SHA alone says nothing there) and it survives the move to checkpoint commits as the documented uncommitted-work fallback. **Neither form is built yet** — like everything else in this ADR they are specified here and implemented in BRIEF-3/4/5.
- **Anchors.** The pinned scope records **both** `HEAD` and `HEAD^{tree}`. [ADR-0069](0069-local-checkpoint-commits.md)'s `/ship` collapse dangles every pre-collapse commit SHA, while the tree hash survives it.
...
|---|---|
| `Round: N` header field | Retires the file-counting heuristic, wrong in three measured ways |
...
| Per-source **verbatim** capture, with synthesis in a labelled layer | Convergence and conflict are findings, not noise to average away |
...
**Naming.** `<YYYY-MM-DD>T<HHMMSS>Z.md`, e.g. `2026-08-09T143211Z.md`. This is a **hybrid — extended date, basic time — and is deliberately not conformant ISO 8601**, because colons are illegal in Windows filenames and the primary development machine is Windows. Do not describe it as ISO 8601 anywhere. UTC with an explicit `Z` is mandatory: local time sorts wrong across a DST boundary and is genuinely ambiguous for one hour every autumn, and the ledger's ordering *is* its index.
...
**No branch-derived component in the filename.** A generated timestamp is a bounded charset with no user-controlled text, which is what makes it immune to the Windows branch-name hazard measured in §9. A human label for a round goes in the file's first heading, never in its name.
...
- Related: [ADR-0069](0069-local-checkpoint-commits.md) — checkpoint collapse; §4's dual anchors exist because the collapse dangles pre-collapse SHAs, and §8 reuses its `/squash-merge` seam
