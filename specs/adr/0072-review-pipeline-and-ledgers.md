...
The repository runs two review loops. The **external** loop is `/land` → `/review-prep` → `/code-review` → `/review-handoff` → `/apply-review`, repeated until findings converge, then handed to the GitHub bot lenses. The **local** loop spawns `spec-reviewer` and `test-reviewer` repeatedly per apply ([ADR-0068](0068-reviewer-isolation-worktrees.md)). Both were built incrementally around a `/code-review` that predates multi-agent runs, and neither skill mentions a briefing step — yet every high-yield pass of the last three months was briefed, by hand, in chat.
...
**So an external round dispatches both, and the round is not complete until both have reported.**
**`ran-no-report` is an incomplete round, not a quiet one.** A lens that ran and left no retrievable
artifact is indistinguishable from one that never ran — this document's founding failure, and the
reason the criterion is *reported* rather than *ran*. A **clean** lens is not that case and is not
blocked by this: a clean run still reports, saying it found nothing, which is exactly the statement
the founding failure is the absence of.
...
| **Which lenses were dispatched**, named, and each one's status — `not-run`, `ran-no-report`, or `reported` | §1 makes an external round complete only when **both** lenses have reported, and without this row a round that dispatched one is indistinguishable from one that dispatched both and got silence from the second — the same shape as the founding failure in Context, reintroduced by the two-lens dispatch. The uncertainty-mapping row below is not a substitute: an empty answers section reads identically whether the adjudication lens ran and found nothing or never ran |
...
**The disk is one leg of that derivation and the retained digest is the other** (added 2026-08-18, when BRIEF-5b implemented the collapse and the
allocation had to survive it). `max(existing)+1` over the branch's directory alone restarts at 1 once a collapse has emptied it, re-issuing numbers
a digest already holds — the case §10 names among the rules enforced by nothing, and the one CodeRabbit raised on PR #95 as two separate Major findings
as evidence that documenting it was not sufficient. So the allocator takes the maximum over the fragments on disk **and** the rounds any digest
recorded having captured from this branch. Both legs are read by `scripts/ledger.py next-round`, which owns the digest's grammar for the same reason
one artifact owns the carrier: the collapse writes those lines and the allocator reads them, and a format restated in two skills is a format that
drifts. Durability's other half is unchanged and procedural: the allocator cannot commit — commits are `/savepoint`'s, with the containment scan and
the enumerated path list that go with them — so the fragment's path is named as owed to the next checkpoint, and the collapse refuses outright on an
uncommitted fragment rather than inlining content that never reached the branch.
...
**Angle record** — the durable half, and the input to every future brief: round number and date, loop, surface, pinned scope (`<base>...<head>` with `<base>` **resolved to a SHA**, `HEAD`, `HEAD^{tree}` — the three anchors of §4), effort level, **each lens dispatched, and its status — `not-run`, `ran-no-report`, or `reported`** (§5 obliges the *report* to state this, but the report has no committed home and §1 makes "both lenses reported" the completion criterion — so unless the fragment carries it, a future round briefing from the ledger cannot tell a round that dispatched one lens from a round whose second lens went silent, which is this ADR's founding failure one layer down; unlike `Fragment path` it is derivable from nothing, so it is knowable only if recorded. **Three states rather than two**, and not because the criterion needs them — §1 makes `not-run` and `ran-no-report` *both* incomplete. It is because their remedies differ: a lens never dispatched is an operator omission that dispatching fixes, where one that ran and left nothing is a tooling or lens failure a re-dispatch may simply repeat. A two-valued field records the two identically and so tells the next round nothing about which it faces), brief revision stamp, **angles briefed**, **angles executed**, **angles briefed but not executed**, **`Examined`** (the surface the round actually reached — see the coverage paragraph below, which is where the field and its reasoning are stated), the do-not-re-run list carried with its evidence and with the paths and commit each entry is scoped to, and the diff size stated against the ~1,000-line detection cliff with the split recommendation if it exceeds it.
...
`digests/<N/100>/pr<N>.md` — **and does not overwrite it**. Idempotence here is *not* rewriting the same bytes; it is that a re-run reaches the same end state without touching a digest that already exists. Measured against the code: `collapse`'s `digest.exists()` branch calls neither `compose_digest` nor `_write_text`. A re-run that finds `branches/<b6>/` already gone finds a digest already there and stops —
...
failed part-way and was retried forty seconds later. A crash *between* those two — digest written, directory only partly deleted — resolves by the same principle but not the same action: **an existing digest is authoritative and is never recomposed.** The re-run finishes the deletion and stops — **unless a fragment present is one the digest never captured, and then it stops without deleting anything**. That case is the invariant two paragraphs down leaving the ground under this rule: "finish the deletion" is safe only while every fragment on disk is one the digest already holds, and where it is not, the documented recovery action is itself the data loss it was written to avoid. The digest records the rounds it captured, so the collapse can *check* that rather than assume it, and a mismatch is a refusal a human reconciles with both artifacts intact. Recomposing from the fragments that happen to remain would overwrite a complete digest with one built from a subset of its own inputs, which is data loss rather than recovery — and nothing about the inlining rule prevents it, since inlining is about not linking to files that are gone, not about completeness. The general rule, stated as the four cases the implementation actually has rather than as an overwrite policy: **a digest is composed and atomically installed only when none exists**; where one exists it is authoritative and is never recomposed or overwritten; a re-entry deletes only fragments whose per-round fingerprint *and* body the digest already holds; and it refuses without deleting anything when any fragment is uncaptured or altered. An authoritative digest whose fragment **directory is already gone** is a true no-op — the run returns without touching the filesystem. If that directory lingers *empty*, the run removes it and says so; that is a write, not a no-op, though it stages nothing because git does not track an empty directory. The distinction is worth the clause: the first draft of this sentence covered both cases as "a no-op" and a reviewer measured the second one calling `shutil.rmtree`. That is safe because of an invariant worth stating rather than leaving implicit: **once a digest exists for a PR, no new fragment is added to that branch's directory** — the branch has merged, and a round briefed against it afterwards would be a round on a different branch with a different `<b6>`. If that ever stopped holding, "finish the deletion and stop" would discard fragments no digest ever captured, which is the same data loss by a different route.

**What a digest holds, and when it lands**. A header of three
fields — the branch hash, the branch name, and the rounds captured, each round number carrying the **fingerprint of the text inlined for it** (`3 (`a1b2c3d4e5f6`)`,
sha256 truncated to twelve hex) — then every fragment inlined in round order with each of its ATX headings demoted one level, so the digest carries exactly
one title and the round records sit under it. The first and third fields are machine-read by the allocator above; the demotion is what keeps the file
MD025-clean, and it is fence-aware because a fragment quotes shell and a `#` inside a fence is content.

**The fingerprints are what make the re-entry rule safe, and recording only the round numbers was not enough**.
The rule below — an existing digest is authoritative, so a re-run finishes the deletion and stops — needs to distinguish a fragment the digest captured from
*the same fragment amended since*, which `/apply-review` does routinely. Round numbers cannot: measured, a re-run deleted a fragment amended after the digest
was written and its content then existed nowhere. Nor can asking whether a fragment's text appears **anywhere** in the digest, which was the first repair and
is sound in the wrong direction — also measured, two rounds whose inlined bodies coincide let round 1's amended content match round 2's captured copy, and the
run deletes it reporting success. A per-round fingerprint answers the question actually being asked. They live on the same line as the round numbers rather
than a line of their own, because two lines carrying one fact is the drift §7 records happening three times to one document.

**The fingerprint is necessary and not sufficient, and the reason is not about hashing**. It is recorded in the digest's
*header*, so it certifies only that the header describes this fragment — and a digest whose header reached disk while its inlined bodies did not answers every
question the re-entry check asks: both contract lines parse, and each fingerprint matches, because the fingerprints were written *with* the header. The
fragments were then deleted against a digest holding no rounds at all. Two changes close it, and they are different in kind. The digest is installed
**atomically** — composed to a staging file and `os.replace`d into position, the same-directory swap [ADR-0047](0047-crypto-surface-implementation-decisions.md) §7
records for the crypto sidecar — so a partial digest is never reachable under the digest's own name. And the re-entry check additionally requires each
fragment's demoted text to be **present in the digest body**, which catches a digest truncated by something other than this module. That substring test is
unsound as the *sole* check, exactly as the paragraph above measures; as a conjunct with the per-round fingerprint it is sound, because a complete digest
inlines each round's text verbatim. A fragment the composer cannot demote at all is reported on this same path as content that cannot be what the digest
holds — never as a formatting error to fix in place, since following that advice edits the one copy of the content the digest does not have. **Inlining preserves a fragment's own relative links only because the two paths sit at equal depth** — `branches/<b6>/round-<N>.md` and
`digests/<N/100>/pr<N>.md` are each two directories below `angle-ledger/` — so a later re-shard that flattens either side breaks every link a fragment
carried and reddens `check_spec_links.py` on a file nobody edited. That is a property of the naming table above rather than a separate rule, and it is
stated here because it is invisible from either path alone.

**The collapse is the branch's last commit, pushed before the merge.** The digest has to be *in* the merged content, and `main` admits no other route:
its history is append-only and PR-mediated, so there is no post-merge commit to put it in, and a follow-up PR per merge would generate the rounds it
was collapsing. The accepted cost is one CI cycle on a head that was already green, which buys the thing the generator obligation below actually rests
on — the digest is lint-checked and link-checked by the PR that introduces it, before it can reach `main`. `/squash-merge` re-verifies checks and the
head OID after that push rather than carrying the earlier green forward, and a branch with no rounds skips the whole step, which is most of them.
...
`/squash-merge` performed no collapse of any kind when this was written (the sizing argument is kept as the reasoning that put it in its own work item). The philosophy is borrowed — branch-local scaffolding folded away at a single seam —
but the implementation is not, and BRIEF-5 sized it as new work — landed by BRIEF-5b as `scripts/ledger.py collapse`. The digest **inlines** fragment content and must not link to it:
...
**The obligation is to the gate, not to a list of rules the composer happens to model**. The validator holds a hand-written model of the rules a *composer* can break, and a rule missing from that model is indistinguishable from a rule that passes: MD001 was absent, so a fragment whose top heading was `##` demoted to `###`, landed directly under the digest's own `#`, and was accepted here and rejected by PyMarkdown — at the moment `/squash-merge` was merging the PR, which is precisely the outcome validating-before-the-write exists to prevent. The title count could not have caught it at any level of care, because a fragment with a `#` and one without contribute the same 0 after demotion. Two consequences worth stating: a new structural rule enabled in the lint config is a change to this composer, and the composer's markdown model — what opens a fence, what closes it, what counts as a heading — must agree with `scripts/check_spec_links.py`, the repository's other parser over the same corpus. Where the two disagreed, they disagreed silently in one direction and produced a false refusal in the other.

**That agreement is currently not held, knowingly, and this sentence is here so a reader of this ADR alone learns it** (2026-08-22). `check_spec_links.py` moved to a real CommonMark parser ([ADR-0061](0061-markdown-link-check-gate.md) §1, revised) and `scripts/ledger.py` kept its `FENCE_RE`, so the pair is no longer two spellings of one rule. Measured at the time of the move, over every markdown file git considers part of the repository: the fence verdict agrees **line for line on 130 of 130**, with no disagreement in the corpus at all. Three shapes the corpus does not contain — a fence indented under a list item, a tab-led fence, and an indented code block — are code to the gate and prose to this composer, and it is tempting to call that the costly direction. Measured, it is not: `ATX_HEADING_RE` admits `{0,3}` leading spaces and no tab, so the very indent that makes those shapes code also stops the demoter matching a `#` inside them, and `demote_headings` returns all three unchanged. The shape that genuinely rewrites a heading inside code is an **HTML block** — masked by the gate, not modelled here at all — where `<!--` / `# comment` / `-->` composes to `## comment`; that shape is *present* in the corpus (7 lines in `specs/adr/0000-template.md`, none of them a heading), so it is live and currently harmless rather than absent. Tracked in [open-questions.md](../open-questions.md) with the trigger that would make it bite. Read the "must agree" above as the standing obligation and this paragraph as its current, measured exception — not as the obligation being withdrawn.
...
Reasoning lives here; the rule is **carried into the skill file that must obey it, citing this ADR**. An ADR-only rule reproduces a failure found three times on one branch: a prohibition "stated in the ADR, enforced by nothing, and invisible to an editor who opens the file directly". Mechanization where it is cheap, per the `tests/test_except_convention.py` precedent: the agent-output-pointer rule is already gated by `tests/test_no_task_output_citations.py`, and a sync check in the shape of `scripts/check_markdownlint_config_sync.py` should pin the self-contained-capture rule into `review-handoff`.

**§8's collapse requirement is mechanized as of BRIEF-5b**: CI's `ledger-collapsed` gate runs `scripts/ledger.py check-collapsed` on every push to `main` and fails if any round fragment is present there. Recorded with its limit rather than as a clean win — it is gated on the ref because uncollapsed fragments are the *expected* state on a branch, so it fires only after a merge that skipped the collapse.

**Three roles, not two**: `/squash-merge` **step 2** is what *prevents*, because it commits and pushes the collapse as the branch's last commit before the merge, which is the account §8 gives; step 5's read-back and this CI gate both only *detect*, step 5 one step earlier than CI. Step 5 cannot prevent anything — it runs after step 4 has already merged, and its own failure path says so, stopping to report rather than repairing `main`. A reader who takes either detector for prevention would be reading it wrong, which is why the distinction is written here rather than left to the step names.

**Five** of this ADR's rules are enforced by **nothing**, and are named here rather than left to read as settled — the third is §7's layout-priority rule, the fourth is §8's rename remedy — move a branch's ledger directory when the branch is renamed, or its fragments orphan silently — and the fifth is the invariant §8 leans on to make "never recompose a digest" safe: that no new fragment is added to a branch's directory once its digest exists. Nothing prevents the addition — a round can still be briefed on a branch whose PR has merged — but **the two consequences that made it dangerous are closed**: allocation reads the retained digest as well as the disk, so it no longer restarts at round 1 against a directory a collapse has just emptied, and the collapse refuses to delete a fragment its digest never captured rather than assuming there is none. The rule stays on this list because detection is not prevention: what a violation now costs is a refusal and a human reconciliation, where it previously cost fragments no digest held. All five count because "an editor remembering it" is not enforcement.

**The containment rule on verbatim capture** (§5) cannot be gated: ADR-0070's enumeration gate keys on the path, and a fragment's path is `specs/reviews/`, so a personal value quoted into a fragment passes every mechanical check the repo has. It is human judgement at capture time, exactly as ADR-0070 concluded for the content half generally — and the ledger is a *new* place that judgement must be exercised, which is the whole reason it is listed.

**Round-number allocation** (§5) is the second: nothing verifies that `N` came from `/review-brief` rather than from a session counting files, which is the heuristic it replaces; the cheap mechanization, if one is ever wanted, is that a fragment whose `N` is not the allocator's own answer for the branch — the maximum over the fragments on disk *and* the rounds its digests captured, plus one — is a detectable error.
...
- [`specs/open-questions.md`](../open-questions.md) — "Where the two branch-keyed carriers live" moves to **Resolved** (§9). Two further changes, both landed by BRIEF-5b (added 2026-08-18, BRIEF-5b round 2 — this manifest had not listed its own branch's entries, and a section whose job is completeness is worth keeping complete): "When does a ledger allocation become durable?" also moves to **Resolved**, since §5's two-leg derivation and `scripts/ledger.py next-round` answer it; and a **new** entry records that nothing constrains the collapse's provenance diff to the three-dot form §8 specifies, with the trigger for resolving it.
...
- `.claude/skills/squash-merge/` — gains the fragment-collapse step §8 specifies. Listed separately because it is the one skill here that gains **new** machinery rather than a rewrite: it performed no collapse of any kind when this was written, so BRIEF-5 sized it as new work — landed by BRIEF-5b as `scripts/ledger.py collapse`, invoked from step 2.
...
- Related: [open-questions.md](../open-questions.md) — "Should ADR-0072's §8 and §10 be consolidated before the ADR is accepted?", now resolved: the structural-only consolidation it settled was applied to §8 and §10 while this ADR was still `Proposed`, so nothing is owed before the `## Status` field changes
