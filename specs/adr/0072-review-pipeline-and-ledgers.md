...
`digests/<N/100>/pr<N>.md` — **and does not overwrite it** (corrected 2026-08-18, BRIEF-5b, from a CodeRabbit finding: this sentence read "and overwrites it", which the implementation never does and which the sentence three clauses down flatly contradicts). Idempotence here is *not* rewriting the same bytes; it is that a re-run reaches the same end state without touching a digest that already exists. Measured against the code: `collapse`'s `digest.exists()` branch calls neither `compose_digest` nor `_write_text`. A re-run that finds `branches/<b6>/` already gone finds a digest already there and stops —
...
**The fingerprints are what make the re-entry rule safe, and recording only the round numbers was not enough** (corrected 2026-08-18, before BRIEF-5b landed).
...
**The fingerprint is necessary and not sufficient, and the reason is not about hashing** (added 2026-08-18, BRIEF-5b round 2). It is recorded in the digest's
...
`/squash-merge` performed no collapse of any kind when this was written (BRIEF-5b built it, 2026-08-18; the sizing argument is kept as the reasoning that put it in its own work item). The philosophy is borrowed — branch-local scaffolding folded away at a single seam —
