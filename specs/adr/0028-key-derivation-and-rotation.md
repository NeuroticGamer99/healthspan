...
2. **Mandatory verified backup first** — the command runs `healthspan db backup`, then *verifies the backup opens with the current credentials*, before touching the live file. `PRAGMA rekey` rewrites every page of the database in place; a crash mid-rekey can corrupt the whole file, and unlike a bad row deletion the blast radius is everything. A `--no-backup` flag exists for scripted expert use and prints what it is skipping.
...
- **Derive once, at Core Service startup.** The user's passphrase is read, the key derived, and the passphrase buffer discarded immediately. Argon2id's cost is paid exactly once per process lifetime.
