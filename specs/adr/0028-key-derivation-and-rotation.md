...
- **Derive-time floor enforcement:** `healthspan init` and rotation already refuse to *write* a `.keyparams` record below the OWASP floor. The reader applies the same floor on the *read* path: before deriving, it validates the sidecar's `m`, `t`, `p` against the recorded OWASP minimums and refuses to derive if any parameter falls below it, printing the same recovery guidance as a missing sidecar (see The `.keyparams` Sidecar, below). The sidecar is not secret, but it is integrity-sensitive — a tampered, corrupted, or hand-edited sidecar that silently weakened Argon2id would otherwise derive successfully; refusing to derive is what makes tampering with it unprofitable.

**Key handoff to SQLCipher** uses the raw-key form:

```python
conn.execute(f"""PRAGMA key = "x'{db_key.hex()}'";""")  # noqa: S608 — ADR-0028 sanctioned
```
...
- **Hold the raw 32-byte key in a `bytearray`**, not an immutable `bytes` object or a `SecretStr`-wrapped `str` — a mutable buffer is what the pool/keying component can actually zero later. A thin wrapper reproduces `SecretStr`'s repr/log-redaction discipline (never logged, never in tracebacks) without giving up mutability. The transient hex form (`.hex()`, for the `PRAGMA key` statement) is produced only at connection-open and discarded immediately — it is never retained as the long-lived representation. This is honest, not lax: ADR-0013's own threat-model table lists memory scraping of the open database as *not protected against* — releasing the key between requests defended nothing while costing a full KDF run or a held key anyway.
...
- **Best-effort zeroization on shutdown**: the `bytearray` is overwritten in place before process exit — a real overwrite, not a symbolic one against an object CPython may have already copied. Still best-effort: the transient hex string and SQLCipher's own internal copy of the key exist beyond this buffer's control and are not zeroized. CPython's copying GC and string interning make guaranteed zeroization of *any* Python object impossible — this is hygiene, not a boundary.
...
- Extended by: [ADR-0047](0047-crypto-surface-implementation-decisions.md) — passphrase advisory, sidecar encoding, keychain entry name, pre-rekey backup locus, exclusive-access timing
