...
`specs/open-questions.md` ("Case-sensitivity of the lint carve-out for the personal path") recorded the `markdown-lint` asymmetry as acceptable, and named the residual surface precisely: "a lint log *naming* a mixed-case recreation". **This change moves where that log lives**, which is a change in the surface rather than in the reasoning, and it is recorded here rather than closed. An earlier draft of this paragraph said the change makes the log "durable rather than transient", and that comparison does not survive contact with the caller this runner is built for: an unspooled gate streams to the console, and the console of an agent session is a transcript that is itself stored under version control and pushed to a remote. Measured — `~/.claude/projects` is a git repository with a configured `origin`. So the pre-change spelling was not transient either, and of the two locations it is the *retained temp file* that is pruned within five runs and never leaves the machine. Whatever else is true here, spooling did not make this worse. Closing it means widening both carve-outs, which is a change to the lint gate ([ADR-0062](0062-markdown-style-lint-gate.md)), to `check_spec_links.py`, and to `ci.yml` — larger than this ADR's scope and owed its own decision.

What bounds it meanwhile is a chain of independent preconditions, **not the single one an earlier draft claimed**. That draft said "the precondition is independently gated: a recreation under the containment directory, in any casing, fails `check_personal_containment.py`, whose own pathspec **is** `:(icase)`." That is true of a *full* run — `containment` is ordered ahead of both lint gates and the runner stops at the first failure, so neither ever executes — and false of a run that selects a gate by name, which this runner supports and which executes no other gate. Raised by Copilot on PR #106, whose comment named the selector case exactly. The corrected bound is the conjunction, each part measured:
