...
`specs/open-questions.md` ("Case-sensitivity of the lint carve-out for the personal path") recorded the `markdown-lint` asymmetry as acceptable, and named the residual surface precisely: "a lint log *naming* a mixed-case recreation". **This change moves where that log lives**, which is a change in the surface rather than in the reasoning, and it is recorded here rather than closed. An earlier draft said the change makes the log "durable rather than transient". The second word is the problem: an unspooled gate streams to the console, and how long *that* survives is a property of whoever is running the gate — a terminal scrollback, a CI job log, a session transcript — none of which this repository controls or can characterize. Calling it transient assumed the most favourable of those. What this ADR can say without leaving the repository is the narrower thing: the retained file's lifetime is bounded and stated here, at five completed runs or the grace period, whichever comes first. Whether that is longer or shorter than the console's is a question about the caller, and it is deliberately not answered here. Closing it means widening both carve-outs, which is a change to the lint gate ([ADR-0062](0062-markdown-style-lint-gate.md)), to `check_spec_links.py`, and to `ci.yml` — larger than this ADR's scope and owed its own decision.

What bounds it meanwhile is a chain of independent preconditions, **not the single one an earlier draft claimed**. That draft said "the precondition is independently gated: a recreation under the containment directory, in any casing, fails `check_personal_containment.py`, whose own pathspec **is** `:(icase)`." Raised by Copilot on PR #106, whose comment named the selector case exactly. **It is not one bound, because the two gates are not ordered alike, and a first attempt at this correction claimed they were.** `markdown-lint` is ordered *behind* `containment`, so a full run stops before reaching it and only a run selecting it by name — which executes no other gate — gets that far. `spec-links` is ordered *ahead* of `containment` and therefore has no such protection at all: it runs first on every full run, and by the time the containment gate would fail, it has already produced whatever output it was going to. That is the sharper half of the two, it was missed in the first correction, and `test_containment_precedes_the_gates_that_lint_the_tree` now pins the relationship so the next reader gets an answer from the registry rather than from prose.
...
- **CI does not reach this code path at all.** `ci.yml` does not invoke `scripts/run_gates.py`; CI runs the gate commands directly, so nothing spools there and nothing is retained.
