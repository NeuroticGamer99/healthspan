...
**The rule for future gates:** a gate that can print a path under the containment directory, or a health value, declares itself unspooled. This is not a judgement call to be made per run; it is a property of what the gate prints when it fails.
...
`specs/open-questions.md` ("Case-sensitivity of the lint carve-out for the personal path") recorded the `markdown-lint` asymmetry as acceptable, and named the residual surface precisely: "a lint log *naming* a mixed-case recreation". **This change is what makes that log durable**, which is a change in the surface rather than in the reasoning, and it is recorded here rather than closed. Closing it means widening both carve-outs, which is a change to the lint gate ([ADR-0062](0062-markdown-style-lint-gate.md)), to `check_spec_links.py`, and to `ci.yml` — larger than this ADR's scope and owed its own decision.

What bounds it meanwhile is that the precondition is independently gated: a recreation under the containment directory, in any casing, fails `check_personal_containment.py`, whose own pathspec **is** `:(icase)`. So this is defence in depth that has thinned, not a control that has failed. ADR-0075 §1 records that nothing detects either pathspec's removal, which applies unchanged.
