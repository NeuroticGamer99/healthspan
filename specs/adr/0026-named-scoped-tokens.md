...
- **`gui` deliberately lacks `admin`:** `admin` carries token administration and the on-demand `backup.database` job ([ADR-0038](0038-backup-execution-and-verification.md)) — operator surfaces the GUI has no business holding. Accepted consequence, not an oversight: the GUI cannot submit `backup.database`, so its pre-delete backup offer degrades to showing the age of the last successful verified backup (job history, `jobs` scope) and pointing at the CLI ([ADR-0027](0027-audit-trail-and-corrections.md)).
...
**Rate limiting:** Failed authentication attempts are rate-limited with exponential backoff (including localhost — a compromised local process brute-forcing token values should hit a wall). Four rules (review item 2.4 — on the default deployment every client is `127.0.0.1`, so a limiter keyed on source address alone would let one misconfigured client lock out every other local client):

1. **The limiter throttles failures only.** A request presenting a valid credential is never delayed or rejected by the limiter — verifying a candidate is one SHA-256 + `compare_digest`, cheap enough to always perform. Brute-force is slowed without ever blocking a legitimate client: a GUI retrying a rotated-out token on a timer is throttled in its own bucket while the MCP Server, CLI, and Automation Host (valid tokens, same address) are unaffected.
2. **Bucket key = (source address, token-name prefix)** parsed from `hsp_<name>_…`. The embedded name is advisory (see Format above), which is sufficient for bucketing: lying about the name only spreads failures across buckets, which rule 3 catches. Credentials with no parseable name share a single `invalid` bucket per address.
3. **Per-address aggregate cap:** total failures across all buckets at one address trip an address-level limit (threshold a multiple of the per-bucket one), so name-cycling does not evade the limiter. Because of rule 1, tripping it still affects only failing requests from that address.
4. **Bounded backoff, visible escape hatch:** backoff caps at a maximum delay (default 60 s, configurable) so a misconfigured client recovers in bounded time once fixed. The operator diagnoses with `healthspan token list` (`last-used`, status) and may clear limiter state immediately with `healthspan auth reset-limits` (requires `admin`) — always reachable, because valid admin credentials are never throttled.

Authentication failure responses are uniform and generic: no distinction between "unknown token" and "revoked token" leaks to the caller. A rate-limited rejection is `429` (revealing that the limiter fired discloses nothing about token state) and is audited with the `rate-limited` outcome below.
...
- **ADR-0027** (Proposed): the delete flow's pre-delete backup offer respects these scopes — `cli-admin` submits `backup.database`; the GUI, lacking `admin`, shows last-backup age instead
...
- Extended by: [ADR-0050](0050-token-store-and-auth-implementation-decisions.md) — bootstrap timing (default set mints at first `service start`, not `init`), token-name charset (no underscore), uniform-401 concretization
