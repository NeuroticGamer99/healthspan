...
**Binding posture.** The default `host` is `127.0.0.1` — loopback only, the local-first default ([ADR-0006](0006-application-architecture.md), [security.md](../security.md)). A LAN binding (`0.0.0.0` or a specific interface) is an explicit, user-typed opt-in; the platform never binds beyond loopback by default. Non-loopback operation is **out of scope for the initial development plan** ([development-plan.md](../development-plan.md), Phases 0–8): the parser accepts the opt-in value, but LAN deployment is *unsupported* until the gated controls in [security.md](../security.md) Network Security (Host-header validation, CORS, HTTPS) land — tracked in [open-questions.md](../open-questions.md), "Non-loopback binding hardening". `port` covers the Core Service alone; other processes (the MCP Server, Phase 4) add their own keys when they arrive rather than presuming a shape now.
...
The Core Service app is built with `openapi_url=None`, `docs_url=None`, `redoc_url=None` — FastAPI's auto-generated `/openapi.json`, `/docs` (Swagger), and `/redoc` are **off**. Two reasons: they would be additional unauthenticated routes (violating the "exactly one `public` route" property decision 4/§enforcement rests on), and an unauthenticated `/openapi.json` publishes the full API surface — fingerprinting material the same driver in [ADR-0040](0040-health-endpoint-authentication.md) ("minimize what an unauthenticated caller can learn") argues against. Whether and how to expose API docs *securely* (behind auth, or `monitor`-gated) is **deferred** — tracked in [open-questions.md](../open-questions.md), Operations, "Authenticated API-docs exposure"; [api-reference.md](../api-reference.md) remains the design-time API reference in the meantime. This does not contradict api-reference.md's note that FastAPI "will produce" OpenAPI at runtime — it defers *when* and *behind what auth*, not *whether*.
...
- Extended by: [ADR-0066](0066-startup-permission-verification.md) — startup permission-verification defaults (posture, Windows depth, override, scope)
...
- Extends: [ADR-0040](0040-health-endpoint-authentication.md) — decision 4 fixes the per-source-address liveness rate cap ADR-0040 requires but leaves unnumbered
- Extends: [ADR-0042](0042-process-supervision-and-single-instance-locking.md) — decision 6 makes its single-instance advisory lock the service-up guard the sanctioned direct-database commands consult
