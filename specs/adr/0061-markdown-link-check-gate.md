...
3. **Crawl all repo markdown — tracked plus untracked-but-not-ignored** (chosen, revised 2026-08-08) vs. **scope to the specs/ tree** (the original choice). The original scoping reasoned that the observed failure mode lived under specs/ and root-level sources could wait "if a need appears." The need appeared: the `.claude/**` tree grew to 17 tracked markdown files — skills and agent definitions whose prose the review pipeline executes as instructions — outside every docs gate, and an audit (finding J8) showed a durable rule copied into those files could rot with CI green. Rather than adding `.claude/` to an enumerated list (the mechanism that just went stale), the source set becomes **every `*.md` git considers part of the repo** — tracked plus untracked-but-not-ignored (`git ls-files --cached --others --exclude-standard`), the same file-set contract as the repo's other tree-walking gates — so a new markdown file is gated from the moment it is written, not the moment it is tracked; `/land` runs this gate *before* staging, which is exactly when a fresh doc's dead links would otherwise escape it. `specs/personal/` remains excluded (gitignored; git never lists it).
