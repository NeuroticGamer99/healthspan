...
- The no-health-values-in-logs prohibition is **platform-independent**: a health value reaching a log is an application-code/fixture defect that manifests identically on every OS. A serial canary run on *either* remaining leg fully guards it — so the two serial legs are a complete backstop, and the parallel leg can never become the prohibition's sole guardian.
...
- The Windows leg scans a *superset* of a serial leg's **captured** stream: the tee'd controller stream (everything a serial `tee` renders) plus the per-worker sink (the captured per-test output xdist suppresses), and fd-level capture adds spawned-subprocess output the old `tee-sys` dropped. (The one exception is out-of-window worker writes — see Negative Consequences.)
...
- Two narrow capture-loss modes exist versus a serial `tee`, both Windows-only and both mitigated by the serial-leg backstop plus this leg going red: **out-of-window worker writes** (an `atexit`/`pytest_sessionfinish`/interpreter-shutdown emission) go to execnet's devnull, not to any report or the controller tee, so no source scans them; and a **hard-crashed worker** loses the in-flight test's buffered fd-capture (a serial `--capture=tee-sys` had already streamed those bytes live), the crash relay carrying only the traceback. Both require a Windows-only, out-of-window-or-crash-correlated leak — a leak the serial legs cannot see only because it is Windows-specific — and the crash reddens the build regardless.
