...
Two integrity constraints belong to the tables they constrain and are recorded in their owning ADRs, not here: the result value-model `CHECK` constraints in [ADR-0030](0030-biomarker-identity.md), and the `framework_ranges` uniqueness constraint with its deterministic point-in-time lookup rule in [ADR-0005](0005-reference-range-frameworks.md). With STRICT and the FK/pragma discipline above, these are the database-level analog of the application's validation boundary.
