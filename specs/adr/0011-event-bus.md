...
- Every event carries a monotonically increasing sequence ID (per Core Service run), sent as the SSE `id:` field in the form `<run-epoch>:<seq>` — the run epoch is an opaque identifier generated fresh at each Core Service start (a UUID is fine), and **equality is its only defined comparison**: no ordering, no "newer epoch" logic (review item 2.2)
...
- On reconnect, a client sends the standard SSE `Last-Event-ID` header. If its epoch matches the current run, the adapter replays retained events after that sequence, then resumes live streaming
...
- If the cursor's epoch does **not** match the current run — or the header is unparseable — the stream begins with a mandatory **all-partition `gap` marker**: the prior run's window is gone wholesale, and the subscriber cannot know what that run emitted after its cursor, so this path fails closed into gap reconciliation. Without the epoch, a prior-run cursor is indistinguishable from a current-run one — an empty replay would report nothing missed (the silent trigger loss ADR-0025 forbids), and once the new run's counter passed the stale sequence, unrelated events would replay as continuations. A client connecting with no `Last-Event-ID` at all is a fresh subscriber: live streaming, no gap, unchanged
...
- The Automation Host persists its last-processed event ID across restarts (ADR-0025) — the full epoch-qualified ID, so a Core Service restart is detected as an epoch mismatch on its next reconnect, never experienced as a silently empty replay
...
- Resolves: architecture review 2026-07-07, item 2.2 — per-run epoch on the SSE cursor; a stale cursor from a prior run forces a `gap`, never silence
