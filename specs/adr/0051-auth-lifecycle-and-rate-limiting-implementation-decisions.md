...
### 4. Revoking the token that authenticates the request is refused

`POST /v1/tokens/{name}/revoke` answers `409` when `{name}` is the requester's own token. `cli-admin` is the only default `admin` holder; bootstrap never re-mints into a non-empty table (ADR-0050 §1) and no direct-database escape hatch exists (the Phase 2 milestone allows none), so self-revocation would be irreversible loss of the entire admin surface. Rotation — which the refusal message points to — covers the compromised-`cli-admin` case without the lockout.
