...
- **Catalog correction (merge and removal)** — decided ([ADR-0065](adr/0065-catalog-merge-and-removal.md)): a same-concept catalog duplicate is repaired by an owner-invoked **merge** — referencing rows re-pointed in place onto the survivor (the two merge-repairable reference columns are declared in the per-table Import-identity section below), the orphan hard-deleted with a full audit image, the merged-away biomarker name aliased to the survivor; any collision refuses the whole merge, which never chooses between conflicting values. **Removal** refuses while observations reference the entry and cascades its own attribute rows, audited
...
- Retention/pruning is deferred to the jobs-table pruning pattern ([ADR-0012](adr/0012-job-abstraction.md), Phase 4a). The unconditional `BEFORE DELETE` trigger above must be reconciled with that mechanism first — see [open-questions.md](open-questions.md), "`auth_audit` retention pruning vs. the shipped append-only trigger".
