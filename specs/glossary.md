...
**Migration 0001**
The first schema migration file, `src/healthspan/migrations/0001_initial_schema.sql` (numbering convention per [ADR-0009](adr/0009-database-migration.md); packaged as installed package data per [ADR-0048](adr/0048-migration-file-packaging.md)). It creates the complete initial schema, including the `audit_log` table and its immutability triggers ([ADR-0027](adr/0027-audit-trail-and-corrections.md)). Landed in Phase 1 WI-3; "before migration 0001" is used throughout the specs as the milestone by which all schema-shaping decisions had to be resolved, since retrofitting them after data exists is far more invasive.
...
**Recovery Kit**
A printable document generated at `healthspan init` containing the secret key as a QR code and Base32 string, with a blank line to handwrite the master passphrase. Enables cross-device database recovery. Useless without the passphrase. Digital renders of the kit are transient artifacts that must not outlive the generating command except by explicit `--output` choice. See [ADR-0013](adr/0013-encryption-at-rest.md) and [ADR-0033](adr/0033-plaintext-artifact-disposal.md).
