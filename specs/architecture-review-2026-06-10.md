...
- [x] `framework_ranges(range_low, range_high)` with no unit is dangerous: an Attia ApoB target in mg/dL compared against a result in g/L silently produces garbage flags. Every range row needs a unit; comparison must unit-normalize. The `effective_date NULL` column already gives option 3 for free when needed. — *Resolved: [ADR-0005](adr/0005-reference-range-frameworks.md) TBD resolved to option 2 (Proposed, freely editable) — mandatory UCUM `unit` column added to `framework_ranges`, comparison must unit-normalize to the biomarker's canonical unit (fails loudly if irreconcilable) and respect the result value model; `effective_date NULL` documented as giving option 3 for free.*

### 3.E Ground the biomarker model in LOINC and UCUM

- [x] Canonical *identifier* = LOINC code (human-readable name as display); labs print different names but report LOINC on most results — collapses the alias problem — *Resolved by [ADR-0030](adr/0030-biomarker-identity.md) (Proposed): internal surrogate `biomarker_id` stays the key (all FKs unchanged), `loinc_code` added as a nullable UNIQUE standard-identifier attribute, canonical name kept as display. Chose attribute over natural key after exploring the natural-key path — it forces synthetic codes for LOINC-less metrics, couples the schema to LOINC's release/deprecation cycle, and fragments the longitudinal series on method-variant codes. Alias claim tempered honestly: LOINC resolves electronic feeds but a name-based fallback is still needed for PDF/manual data.*
- [x] Units as UCUM strings, with a canonical unit per biomarker — gives unit conversion a standard to validate against — *Resolved by [ADR-0031](adr/0031-units-and-ucum.md) (Proposed): units stored as UCUM strings, `canonical_unit` per biomarker, all comparison unit-normalized. Conversion engine left as an explicit open sub-decision (ucumvert is the only credible Python option — MIT, maintained, but 0.x and one-directional; pyucum is dead + calls remote APIs; ucum-lhc is JS) to be resolved before implementation — split into its own ADR per that research.*
- [x] Makes ADR-0018's FHIR work nearly free (`Observation.code` *is* LOINC) — *[ADR-0018](adr/0018-fhir-interoperability.md) cross-linked to ADR-0030/0031: `Observation.code` comes from `loinc_code`, `Quantity.code` from the UCUM unit, `valueQuantity.comparator` from the value model.*
- [x] Plan value representation for real lab data now: numeric value + comparator (`<0.1` below-detection results) + text value for qualitative results — FHIR's `valueQuantity.comparator` model is the proven shape; a bare `REAL` column fails on the first thyroid antibody panel — *Resolved by [ADR-0030](adr/0030-biomarker-identity.md): `value_num` + `comparator` (`<`/`<=`/`>=`/`>`, NULL = exact) + `value_text` (qualitative). Below-detection `<0.1` stored as `value_num=0.1, comparator='<'`, never conflated with a measured 0.1.*

*Follow-up raised during this work: one biomarker concept (e.g. LDL-C direct vs. calculated) can map to several LOINC codes. Captured as [ADR-0032](adr/0032-biomarker-loinc-cardinality.md) (Proposed — stub) to reason through reported-LOINC-per-result vs. a mapping table vs. method-as-a-dimension before multi-lab electronic-feed import.*
