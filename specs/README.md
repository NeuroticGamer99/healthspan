...
| [molar-mass-provenance.md](molar-mass-provenance.md) | Provenance record for the seeded `biomarkers.molar_mass` values: per-value source tiers, the counter-intuitive values explained, and the outstanding tier-B verification action ([open-questions.md](open-questions.md)). |
| [manual-entry-quickstart.md](manual-entry-quickstart.md) | How to run the platform from a checkout (pre-distribution): bootstrap sequence, Recovery Kit and backup discipline, the one-database-many-checkouts rule, and what to record while exercising manual entry. |
