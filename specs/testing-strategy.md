...
- Process supervision (ADR-0042): a keyless child (Automation Host / MCP Server) killed unexpectedly is restarted with exponential backoff and a `system.*` event; a child that crash-loops (5 restarts within the rolling 60 s window) trips the circuit breaker — the launcher stops retrying, emits a `system.*` event naming it, and status reports the degraded state; in interactive/passphrase mode a Core Service exit does **not** trigger a silent restart (no retained passphrase) and surfaces loudly, while in full-auto-unlock mode Core Service is restarted and re-derives from the keychain
