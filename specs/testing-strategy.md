...
- Catalog merge and removal ([ADR-0065](adr/0065-catalog-merge-and-removal.md)): **collision refusal is atomic** — a merge with any colliding current result, framework range, or draw writes nothing (no partial re-point, no audit rows) and reports every colliding pair, not just the first; a clean merge re-points **all** referencing rows including superseded chain members (no row still references the orphan afterward — the orphan delete succeeds under `foreign_keys=ON`), writes one `update` audit row per re-pointed row with images and the auto `reason`, deletes the orphan with a full image, and only then inserts the merged-away name as a survivor alias — the ADR-0054 §3 namespace check would reject the insert while the orphan lives — asserting exactly one `insert` audit row when the alias is written, zero **alias** audit rows when an equal normalized alias already points at the survivor (the merge's re-point and delete audit rows are unaffected), and refusal when the alias points at a third biomarker; **merge never touches a value row's content** — value columns and stored units are byte-identical before and after; removal refuses while any result/draw references the entry (current or superseded) and cascades the entry's own aliases and framework ranges with per-row `delete` audit images, all in one transaction; both endpoints demand `write` **and** `import` — a token holding either scope alone receives 403, asserted for both real single-scope shapes (`write` without `import`: the `automation-host` opt-in; `import` without `write`: the `watch-import` token); and a dry-run — the collision preflight or removal's cascade manifest — writes nothing (no data mutation, no audit rows), with the subsequent real execution still applying the full mutation set
...
- **Startup permission verification ([ADR-0066](adr/0066-startup-permission-verification.md))**: a database, sidecar, backup directory, or config file found broader than owner-only at Core Service start or on a sanctioned direct-database command is restored to owner-only, the finding is reported, and a second start is silent; a repair that cannot be applied degrades to a warning and still lets the command run; a `passphrase_file` readable beyond its owner **refuses** startup, is left untouched, and the error names the rotation obligation — for both the config key and the `--passphrase-file` flag; a `db restore` source pair and the archive swept after `db backup` get the same repair; an undeterminable check (enumeration failure, unreadable stat) reports clean rather than blocking; no flag or config key turns the check off
...
**Canary rule.** Every synthetic health value must be grep-distinctive, because the log canary gate (see [CI Gates](#ci-gates)) works by scanning captured log output for fixture values — and realistic values like a glucose of `95` collide with timestamps, ports, and status codes. Text fields (clinical notes, medication names in notes) embed a `CANARY-` marker token; numeric health values use high-entropy decimals with at least six significant digits (e.g. `104.73921`) that cannot collide with infrastructure numbers. The fixture loader derives the **canary manifest** — the complete list of health values present in the fixtures — programmatically from the parsed typed records (not the raw file text), so there is no hand-maintained list to drift out of sync, and a value reachable only through parsing is still covered. The loader carries a declared registry of the grep-distinctive owner-health columns — every free-text field (which contributes its `CANARY-` tokens) and every high-entropy-decimal metric; catalog, reference-range, provenance, and timestamp values are not manifest values, and integer-typed metrics (e.g. wearable step or heart-rate counts) fall outside by construction because an integer count cannot be made grep-distinctive. The loader also enforces the numeric half of this rule at derivation time — a numeric health value that is not a high-entropy decimal fails the build — mechanizing the fixture-review half of the gate.

### Fixture management

Fixtures live in `tests/fixtures/` as JSON files — each an object mapping table name to a list of row objects, merged across files. (The earlier "JSON or SQL" was narrowed to JSON: raw SQL cannot be parsed back into the typed records the canary-manifest derivation depends on.) The fixture loader (`tests/fixture_loader.py`) creates an ephemeral SQLCipher database, applies all migrations, and loads fixtures in foreign-key-safe order; it also owns the canary manifest the log canary gate consumes (`scripts/scan_log_canary.py` imports it rather than re-deriving from raw text).
...
- **OS keychain**: `keyring` backend differs per platform (DPAPI, macOS Keychain, libsecret). Tests that exercise key storage **mock the backend** — `keyring.testing`, or the autouse in-memory fixture in `conftest.py` — on every platform. Running them against the *real* per-platform backend is not an alternative: `conftest.py` sets `PYTHON_KEYRING_BACKEND=keyring.backends.fail.Keyring` unconditionally at import ([ADR-0068](adr/0068-reviewer-isolation-worktrees.md) §3), so any path reaching real backend resolution raises `NoKeyringError` on all three platforms. That is deliberate — this repo is developed on a machine holding a real encrypted health database — and it is what makes the platform-specific backends untested by construction here. A test that genuinely needs a live backend must call `keyring.set_keyring(<real backend>)` explicitly, which bypasses `load_env()` and therefore the belt; **nothing in the suite passes a real backend**, and adding one needs a decision recorded, not just a fixture.

  **`set_keyring()` is not the only way past the belt, and the absolute above reads as though it were.** The belt is an *environment variable*, so **any child process launched with `PYTHON_KEYRING_BACKEND` rewritten resolves whatever that value names** — `set_keyring` is not involved and the parent's belt does not travel. `tests/test_keychain_isolation.py` does exactly this on purpose: its `_probe` helper spawns a fresh interpreter with the variable overridden, because overriding it *is* the behavior under test. That is the sanctioned use; the hazard is a future author adding a subprocess probe, following the sentence above, setting the variable to a real backend in a child's environment believing the belt still covers it, and reading this machine's live credential vault — the incident [ADR-0068](adr/0068-reviewer-isolation-worktrees.md) §4 records as having already happened here once. Those probes name a *generated* unresolvable module for that reason, and build the child's environment from a curated allowlist rather than inheriting `os.environ`. Treat "a subprocess whose `PYTHON_KEYRING_BACKEND` is not the fail backend" as needing the same recorded decision a `set_keyring(<real backend>)` call would.

  The call itself is not the escape, and an earlier version of this bullet said it was — which mis-described the suite in the sentence defining its own sanctioned exception. `conftest.py`'s autouse `fake_keychain` fixture calls `keyring.set_keyring()` twice per test, installing the in-memory fake and restoring the previous backend; that *is* the mandated mocking, on all ~1,300 tests. What follows is worth knowing when reasoning about the belt: because `set_keyring()` assigns `keyring.core._keyring_backend` directly, `load_env()` — and therefore `PYTHON_KEYRING_BACKEND` — is consulted once per process and never again inside a test body. The belt's live coverage is the window before the first fixture runs, plus every subprocess a test spawns; the fixture owns everything in between.
- **File permissions**: `chmod 600` behavior differs on Windows. Tests that validate config file or database file permissions must account for platform differences. The startup permission check ([ADR-0066](adr/0066-startup-permission-verification.md)) is two implementations, not one — POSIX mode bits and a Windows `icacls` principal enumeration — so a test that only sets a mode exercises one of them: the Windows half needs a real ACL or a stubbed enumerator, and mode-bit tests must be skipped rather than expected to fail there.
- **Path handling**: forward vs backslash; tests should use `pathlib.Path` consistently.
- **Filename encoding**: Linux accepts arbitrary bytes in a filename; **macOS enforces UTF-8 validity at the filesystem layer** and fails the create with `OSError: [Errno 92] Illegal byte sequence` — as does a Linux checkout on a mounted NTFS/DrvFs volume. The *platform* therefore does not decide whether a non-UTF-8 name can exist, so a test that creates one must **probe and `pytest.skip` at the raise site** rather than gate on `skipif(os.name == "nt")`, which lets it run on macOS and fail there. `tests/test_review_worktree.py` learned this by shipping it (PR #78: two tests green on Windows and WSL, red on `macos-latest`) and now carries the probe at all four sites; `_filesystem_is_case_insensitive` in the same file states the general form — probe the property, do not infer it from `os.name`, because that guard gates the wrong hosts in both directions.
- **Running the other platform's half locally**: platform-gated tests **skip**, they do not fail, so a single-platform run reports green while the other half goes unexercised — a Windows run of the ADR-0066 suite passes with the POSIX mode-bit tests skipped, and the POSIX CI legs are where a fixture written at the default umask (0644) first fails. **"The other half" is two legs, not one**, and this bullet previously read as though it were one: the local WSL run is *Linux*, while `macos-latest` is represented by nothing on a Windows development machine. Windows and WSL both green — even with their pass-plus-skip counts summing equal, which is the check that proves no test was skipped on both — says nothing whatever about macOS. PR #78 shipped red on `macos-latest` after exactly that two-leg check reported green (see **Filename encoding** above). On Windows the POSIX half runs under WSL, but **`UV_PROJECT_ENVIRONMENT` is required**: without it `uv run` rebuilds the shared `.venv` with a Linux layout, after which the Windows side cannot delete the `lib64` symlink and needs `rm -rf` from the WSL side to recover. With it, the two environments coexist and neither rebuilds (no `.gitignore` entry needed — `uv` writes one inside every environment it creates).

  ```bash
  wsl -e bash -c 'export PATH=$HOME/.local/bin:$PATH && export UV_PROJECT_ENVIRONMENT=.venv-wsl \
    && cd /mnt/c/path/to/healthspan \
    && uv run --locked --with pytest --with pytest-xdist pytest -n auto -q'
  ```

  Worth doing for any change that branches on `os.name` or `sys.platform` — file modes, ACLs, path handling, subprocess tooling — before pushing, rather than spending a CI round to learn it. A git worktree also isolates the environment, and a bare `git worktree add` sees only *committed* state — but a worktree materialized from a `git stash create` snapshot carries the uncommitted **tracked** state too ([ADR-0068](adr/0068-reviewer-isolation-worktrees.md), which verified the mechanism and mechanized it as `scripts/review_worktree.py`), so the committed-state objection rules out only the bare form. Untracked files are in neither: they need the launcher's explicit copy-in, and a brand-new never-`git add`-ed test file is exactly what a hand-rolled snapshot worktree silently misses. For this pre-push check, run in the live tree with `UV_PROJECT_ENVIRONMENT` as above. `scripts/review_worktree.py` is **not** the tool for it: it is shaped for a reviewer round, creating one worktree *per agent* and recording a per-agent venv path each reviewer's own `uv` run then builds (setup reserves the path; it never creates the directory), emitting Windows-absolute redirect paths that mean nothing inside WSL, and leaving a state file behind. That last one is a wedge, not a degradation: the next reviewer round's setup **refuses loudly** — exit 1, naming the file and the fix — and `.claude/reviewer-isolation.md` files a leftover state file under *Recoverable — fix and rerun, do not fall back*. Run teardown; do not read it as a reason to drop that review to the sequential fallback. If a POSIX check genuinely needs isolation, run the launcher only with a matching teardown on every exit path, exactly as `.claude/reviewer-isolation.md` requires of a review.
...
- the OS keychain is guarded twice in `conftest.py` — an autouse in-memory backend fixture, and an unconditional import-time `PYTHON_KEYRING_BACKEND=keyring.backends.fail.Keyring` assignment that makes any path reaching a real backend raise rather than touch the OS store ([ADR-0068](adr/0068-reviewer-isolation-worktrees.md) §3, pinned by `tests/test_keychain_isolation.py`) — and xdist workers are separate processes, so each has its own;
...
- fixed-path references (`~`, platformdirs) are assertion-only or monkeypatched — no test writes outside `tmp_path`.

The one exception to the last point is worth stating rather than eliding, because eliding it is how a reader concludes the suite is safe for a reason that is not true: `tests/test_review_worktree.py` **does** mutate the working directory, in its `repo` fixture (`monkeypatch.chdir`) and in a handful of tests directly. (Deliberately not a count: the clause this replaced — "and no test mutates the working directory" — was suite-wide and checkable with one grep, and trading it for a hand-maintained number put an unverifiable figure in the paragraph that states the suite's parallel-safety invariant. What matters is the file, not the tally.) It has to — `scripts/review_worktree.py` discovers the repository with `git rev-parse --show-toplevel` against the process cwd, so without the chdir every test would target the developer's real checkout. That is safe under xdist because workers are separate *processes*, which is the actual guarantee; it is not safe under any scheme that threads tests through one process, and it is a live hazard within a worker (`conftest.py` pins `CanaryCaptureSink`'s path at configure time for exactly this reason). A test that doubles that module's git seam must restore it by hand rather than call `monkeypatch.undo()`, which would also undo the fixture's chdir.
...
### CI: the windows-latest leg parallel, the other legs serial ([ADR-0063](adr/0063-parallel-ci-test-execution.md))

The three matrix legs run concurrently, so total CI latency is the slowest leg — and `windows-latest` is the ~3× outlier (its subprocess-spawn cost, above, is heaviest on Windows). That leg alone runs `pytest -n auto`; `ubuntu-latest` and `macos-latest` keep the serial invocation (`--capture=tee-sys --log-cli-level=DEBUG` streamed into one file) untouched.

The serial legs feed the mandatory log-canary gate (below) exactly as before. The Windows leg cannot use that path directly — under xdist each worker's stdout *is* the execnet control channel, and the controller echoes captured output only for failing tests — so it scans two complementary sources: the controller's own stream, tee'd like the serial legs (node-id lines, the failures/errors summary with tracebacks, warnings, xdist crash relays), plus a per-worker sink (`tests/conftest.py`, active only when `CANARY_CAPTURE_DIR` is set) that recovers each passing test's captured stdout/stderr/log. The sink uses **fd-level capture**, which records in-process *and* spawned-subprocess output that `tee-sys` drops onto the execnet channel. Together the two sources scan a *superset* of a serial leg's captured stream, with two narrow exceptions noted in [ADR-0063](adr/0063-parallel-ci-test-execution.md) — out-of-window worker writes (`atexit`/shutdown) and a hard-crashed worker's buffered loss; worker files are created lazily, so a dead sink leaves the glob empty and the scan fails closed.

Parallelizing only the outlier is deliberate: the log-canary gate's prohibition (never log health data values — [observability.md](observability.md), [security.md](security.md) § Logging) is platform-independent in practice — health values flow only through cross-platform logic, so a leak surfaces on every OS — and the two untouched serial legs therefore completely backstop any platform-independent leak. The residual they do *not* cover — a leak on a Windows-specific code path *and* through the parallel leg's narrow capture-loss modes — is an accepted risk documented in [ADR-0063](adr/0063-parallel-ci-test-execution.md), which also covers the capture rework, the measured payoff, and why the other legs stay serial.
...
The CI workflow ([`.github/workflows/ci.yml`](../.github/workflows/ci.yml)) runs the full gate set. Two gates are meaningful against a specs-only tree and ran from the start: repository secret scanning (below) and the ADR-index docs-consistency check ([ADR-0045](adr/0045-repository-workflow-and-ci-enforcement.md), which also decides the enforcement mechanics around this section — main-branch ruleset, the `ci-ok` aggregate required check, merge policy, and the runner matrix). ([ADR-0061](adr/0061-markdown-link-check-gate.md) later added a spec-link check to that same docs-consistency job.) The remaining gates — log canary, strict static typing, lint + format, dependency vulnerability audit — joined the workflow in the first code PR (Phase 0, [development-plan.md](development-plan.md)), each tool's version pinned in that same PR's `env` block (the pins item 3.A deferred to "the CI workflow itself"); that PR also activated the main-branch ruleset per ADR-0045. Landing the gates with the first code they guard was the point — the [2026-07-07 review](reviews/architecture-review-2026-07-07.md) (item 2.5) flagged the retrofitting trap, and shipping them together is what avoided it.
...
2. A final CI step scans everything captured against the canary manifest, which the fixture loader derives from the parsed fixtures (see the canary rule under Fixture design).
...
- **`S` (full bandit family)** — subsumes the original S608-only gate. Mechanizes security.md's "No SQL statement in the codebase may be constructed by string interpolation or concatenation of user-supplied values ... enforced by code review convention and, where possible, by linting". Sanctioned S608 suppressions are annotated inline at the call site (never exempted at the file or project level, so any other S608 hit in the same file still fails), and there are exactly two: (1) in first-party code, [ADR-0028](adr/0028-key-derivation-and-rotation.md)'s raw-hex `PRAGMA key`/`PRAGMA rekey` handoff, where only locally generated, format-validated hex is interpolated; (2) in the test-only fixture loader (`tests/fixture_loader.py`), the parameterized-`INSERT` builder, where table and column identifiers come solely from a static `TABLE_ORDER` allowlist and identifier-validated keys of trusted committed fixtures, and every value is bound — no user input reaches the statement.
