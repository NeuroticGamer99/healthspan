...
- **Running the other platform's half locally**: platform-gated tests **skip**, they do not fail, so a single-platform run reports green while the other half goes unexercised — a Windows run of the ADR-0066 suite passes with the POSIX mode-bit tests skipped, and the POSIX CI legs are where a fixture written at the default umask (0644) first fails. On Windows the POSIX half runs under WSL, but **`UV_PROJECT_ENVIRONMENT` is required**: without it `uv run` rebuilds the shared `.venv` with a Linux layout, after which the Windows side cannot delete the `lib64` symlink and needs `rm -rf` from the WSL side to recover. With it, the two environments coexist and neither rebuilds (no `.gitignore` entry needed — `uv` writes one inside every environment it creates).
  ...
  Worth doing for any change that branches on `os.name` or `sys.platform` — file modes, ACLs, path handling, subprocess tooling — before pushing, rather than spending a CI round to learn it. A git worktree also isolates the environment but only sees *committed* state, which is the wrong shape for a pre-push check.
