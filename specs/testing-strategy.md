...
- Catalog merge and removal ([ADR-0065](adr/0065-catalog-merge-and-removal.md)): **collision refusal is atomic** — a merge with any colliding current result, framework range, or draw writes nothing (no partial re-point, no audit rows) and reports every colliding pair, not just the first; a clean merge re-points **all** referencing rows including superseded chain members (no row still references the orphan afterward — the orphan delete succeeds under `foreign_keys=ON`), writes one `update` audit row per re-pointed row with images and the auto `reason`, deletes the orphan with a full image, and only then inserts the merged-away name as a survivor alias — the ADR-0054 §3 namespace check would reject the insert while the orphan lives — asserting exactly one `insert` audit row when the alias is written, zero audit rows on the no-op branch (an equal normalized alias already pointing at the survivor), and refusal when the alias points at a third biomarker; **merge never touches a value row's content** — value columns and stored units are byte-identical before and after; removal refuses while any result/draw references the entry (current or superseded) and cascades the entry's own aliases and framework ranges with per-row `delete` audit images, all in one transaction; both endpoints demand `write` **and** `import` — a token holding `write` alone (the `automation-host` opt-in shape) receives 403
