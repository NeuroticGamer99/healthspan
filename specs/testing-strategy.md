...
Parallelizing only the outlier is deliberate: the log-canary gate's prohibition (never log health data values — [observability.md](observability.md), [security.md](security.md) § Logging) is platform-independent — a health value in a log is a code/fixture defect that surfaces on every OS — so the two untouched serial legs remain a full backstop, and the change's risk is confined to the one leg that needs the speedup. See [ADR-0063](adr/0063-parallel-ci-test-execution.md) for the capture rework, the measured payoff, and why the other legs stay serial.
