...
The CI workflow ([`.github/workflows/ci.yml`](../.github/workflows/ci.yml)) exists and runs the two gates that are meaningful against a specs-only tree: repository secret scanning (below) and an ADR-index docs-consistency check ([ADR-0045](adr/0045-repository-workflow-and-ci-enforcement.md), which also decides the enforcement mechanics around this section — main-branch ruleset, the `ci-ok` aggregate required check, merge policy, and the runner matrix). The remaining gates are specified but deliberately not yet wired. The [2026-07-07 review](reviews/architecture-review-2026-07-07.md) (item 2.5) flags the trap in that — the first PR that introduces code or a real runtime dependency **must add the remaining gates to that workflow** (log canary, strict static typing, lint + format, dependency vulnerability audit), with each tool's version pinned in that same PR (the pins item 3.A deferred to "the CI workflow itself"); the same PR activates the main-branch ruleset per ADR-0045. Landing the gates with the first code they guard is the point; retrofitting them against an existing codebase is the exact failure mode the gates exist to prevent.
